...
fn decode(snippet: &Snippet) -> (Vec<Instr>, Vec<iced_x86::Instruction>) {
    let mut instrs = vec![];
    let mut iced = vec![];
...
fn main() {
    ...
    let (instrs, iced) = decode(&P3);
    let mut blocks = blocks(instrs, &iced);

    //blocks.vec.truncate(5);

    let hints: HashMap<u32, &'static str> = P3.hints.iter().copied().collect::<HashMap<_, _>>();
    let ctx = ssa::Context {
        iced: &iced,
        hints: &hints,
    };
