use crate::codegen::{CodeGen, Writer, gen_addr, get_mem, get_op, mem_size, op_size, set_op};
...
impl<'a> CodeGen<'a> {
    pub fn codegen_fpu(&self, w: &mut Writer, instr: &iced_x86::Instruction) -> bool {
        use iced_x86::Mnemonic::*;
        match instr.mnemonic() {
            Fld => {
                let expr = fpu_get_op(instr, 0);
                w.line(format!("ctx.cpu.fpu.push({expr});"));
            }
            Fild => {
                w.line(format!(
            ...
            Fldz => w.line("ctx.cpu.fpu.push(0.0);"),
            Fld1 => w.line("ctx.cpu.fpu.push(1.0);"),

            Fst | Fstp => {
                w.line(fpu_set_op(instr, 0, fpu_get_reg(0)));
                if instr.mnemonic() == Fstp {
                    w.line("ctx.cpu.fpu.pop();");
                }
            }
            ...
            Fnstsw => {
                assert_eq!(instr.op_count(), 1);
                w.line(set_op(instr, 0, "ctx.cpu.fpu.status()".into()));
            }

            Fpatan => {
                w.line("let t = ctx.cpu.fpu.get(0);");
                w.line("ctx.cpu.fpu.pop();");
                w.line("ctx.cpu.fpu.set(0, ctx.cpu.fpu.get(0).atan2(t));");
            }
            _ => return false,
        }
        true
